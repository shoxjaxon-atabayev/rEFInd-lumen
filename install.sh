#!/usr/bin/env bash
# Lumen — installer for a minimal rEFInd boot theme with colour variants.
#
...
#   <refind-dir>/themes/lumen/     — the theme payload
...
THEME_SUBDIR="themes/lumen"                   # must match the paths inside theme.conf
INCLUDE_LINE="include ${THEME_SUBDIR}/theme.conf"
MARKER_BEGIN="# BEGIN Lumen (managed by install.sh — do not edit this block by hand)"
MARKER_END="# END Lumen"
DEFAULT_COLOR="white"
DEFAULT_BG="dark"
...
# sha256 of tools/generate.py's own bytes — must match generate.py's own
# GENERATOR_FINGERPRINT exactly (same hash, same file, same bytes) so
# is_cached() can tell a build/colors/<colour> generated by an older version
# of the script apart from one made by what's on disk now. Cached after the
# first call since it shells out to python3.
generator_fingerprint() {
  if [ -z "$GENERATOR_FINGERPRINT_CACHE" ]; then
    GENERATOR_FINGERPRINT_CACHE="$(python3 -c '
import hashlib, sys
print(hashlib.sha256(open(sys.argv[1], "rb").read()).hexdigest())
' "$GENERATE_PY")" || die "failed to fingerprint $GENERATE_PY"
  fi
  printf '%s\n' "$GENERATOR_FINGERPRINT_CACHE"
}
...
# backgrounds) generated by the CURRENT tools/generate.py — lets
# ensure_generated() skip a colour that's already sitting on disk from an
# earlier run instead of needlessly rebuilding it. The .fingerprint check
# matters as much as the file-existence ones: without it, a build/colors/
# left over from before some rendering fix (a card/frame geometry change, a
# palette tweak, ...) reads as "already generated" forever by existence
# alone, so `install.sh --color X` would keep silently shipping the old,
# pre-fix render of X instead of noticing generate.py has changed.
is_cached() {
  ...
  [ -f "$d/selection_big.png" ] && [ -f "$d/selection_small.png" ] \
    ...
    && [ -f "$d/.fingerprint" ] && [ "$(cat "$d/.fingerprint")" = "$(generator_fingerprint)" ]
}
...
validate_source() {
  ...
  [ "$missing" -eq 0 ] \
    || die "theme source is incomplete — run this from inside the refind-lumen checkout."
...
do_install() {
  ...
  if [ ! -f "$conf" ]; then
    ...
    if [ -r /usr/share/refind/refind.conf-sample ]; then
      ...
      printf '# created by Lumen install.sh\ntimeout 20\n' > "$conf"
  ...
  if cmp -s "$tmp" "$conf"; then
    ...
    backup="$(unique_path "${conf}.lumen-backup")"
    cp "$conf" "$backup"
  ...
  trap - EXIT
  step "done"
  ok "Lumen installed · colour: $COLOR · background: $BG"
...
do_uninstall() {
  ...
  trap - EXIT
  step "done"
  ok "Lumen uninstalled"
  info "refind.conf backups (*.lumen-backup-*) were left for you to delete"
}
...
if [ ! -d "$SCRIPT_DIR/icons" ] || [ ! -f "$GENERATE_PY" ]; then
  die "icons/ or tools/generate.py missing here — run this from inside the refind-lumen checkout."
fi
